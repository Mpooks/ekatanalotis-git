...
if (mysqli_num_rows($findm) > 0) {
                ...
                while ($to = mysqli_fetch_assoc($tot)) {
                    ...
                    while ($d = mysqli_fetch_assoc($dd)) {
                        ...
                        while ($ldd = mysqli_fetch_assoc($ld)) {
                            ...
                            $ltok = $tobrspl * $mscr / $v;
                            $upUS = mysqli_query($con, "UPDATE users SET total_score=total_score+monthly_score,last_tokens=$ltok,total_tokens=total_tokens+$ltok WHERE id=$uid");
                        }
                    }
                }
            }
    ...
    $addm=mysqli_query($con, "INSERT INTO updatedMonth VALUES ('$thismon','NO')");
}
closeDB($con);
